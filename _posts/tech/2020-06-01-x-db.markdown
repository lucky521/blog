...
永恒的存储需求。和计算机永恒的计算需求一样，存储需求也是，并且可以肯定的是，随着数字化/AI/Agent的发展，存储需求越来越大，人类文明无论集体还是个体，太多的信息量需要存储。
...
HDFS是Hadoop的存储系统，它的优点是可以存储超大量数据，但是缺点是速度慢。 HBase在HDFS中存储的数据文件称为HFile。这些HFile文件是HBase用来存储表数据的底层文件格式。HFile是HBase的核心存储格式，设计用来高效地进行随机读写操作。
...
[Hbase官方手册](https://hbase.apache.org/book.html)

HBase中的数据是按照Rowkey的ASCII字典顺序进行全局排序的
scan查询，用于读取多行数据。可以指定起始行键和结束行键来定义扫描的范围。


查询结果保序
https://hbase.apache.org/book.html#dm.sort
...
存储接口标准： 
* AWS S3标准协议 (Amazon Simple Storage Service (Amazon S3) )
* POSIX标准
