...
* cp失败的时候是否抛异常？setFailOnCheckpointingErrors
...
* 3. OffsetCommitMode.KAFKA_PERIODIC: 使用Kafka的internal client的默认行为，周期性将offset commit到kafka 如果不想借助checkpoint来重置kafka的offset，可以利用 FlinkKafkaConsumerBase 设置 setCommitOffsetsOnCheckpoints(false)，以及在kakfa properties里面设置 “auto.commit.enable” 为false，这样就相当于没有commit offset，作业恢复的时候，如果你们设置是从kafka consume from latest，既可以恢复checkpoint中的state（你们应该是想要主要恢复keyed state相关吧），也可以从最新的offset消费。
...
* state.backend.local-recovery 此选项配置此 state backend 的本地恢复。
...
问题：Checkpoint失败与作业失败的关联？
如何应对 Exceeded checkpoint tolerable failure threshold ？
